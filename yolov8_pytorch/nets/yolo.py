...
import os
# 修改前的错误路径计算
# project_root = os.path.dirname(os.path.dirname(r"D:\YOLO\YOLOv8-Sgbm\yolov8_pytorch"))

# 修改后的正确路径计算
current_script_path = os.path.abspath(__file__)  # 使用__file__获取当前脚本路径
project_root = os.path.dirname(os.path.dirname(current_script_path))  # 向上回退两级到项目根目录
sys.path.append(project_root)

# 现在可以正确导入 nets.backbone
...
class YOLOXHead(nn.Module):
    def __init__(self, num_classes, in_channels, depths=[2, 2, 2]):
        super().__init__()
        self.num_classes = num_classes
        self.anchors_per_location = 3  # 每个特征图的锚点数量

        # 确保输出通道计算正确
        output_channels = self.anchors_per_location * (5 + num_classes)
        ...
        for _ in range(depths):
            self.conv_layers.append(
                nn.Sequential(
                    ...
                    nn.Conv2d(in_channels, output_channels, 1)  # 关键修正点
                )
            )
